...
import org.efaps.util.RunLevel.CacheMethod;
...
public class Cache<K extends CacheObjectInterface> {
  ...
  public static void reloadCache() throws Exception {
    synchronized (caches) {
      ...
      /*
       * JAASSystem.initialise(_context); Role.initialise(_context);
       * Group.initialise(_context); AttributeType.initialise(_context);
       * SQLTable.initialise(_context); Type.initialise(_context);
       * Attribute.initialise(_context); AccessType.initialise(_context);
